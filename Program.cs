...
public class Program
{
    static void Main()
    {
        ...
        const string MessageInicialSwag = "You have defeated the drac and you have found a mine full of magical bitcoins!";
        const string MessageExcavation = "Excavation {i}: You have found {bits} bits!";
        const string MessageDontFindAnything = "Today it's not youre day of lucky, you find 0 bits";
        const string MessageTotalBits = "Total bits obtained: {totalBits}";
        const string MessageBigger200 = "You've unlocked the gold GPU! Your spells now run at 120 FPS!";
        const string MessageSmaller200 = "Your magic card is still integrated. It's time to defeat another dragon!";
        ...
        do
        {
            ...
            switch (op)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    Console.WriteLine(MessageInicialSwag);
                    int totalBits = 0;

                    for (int i = 1; i <= 5; i++)
                    {
                        bool found = random.Next(0, 100) > 5;

                        if (found)
                        {
                            int bits = random.Next(5, 51);
                            totalBits += bits;
                            Console.WriteLine(MessageExcavation);
                        }
                        else {
                            Console.WriteLine(MessageDontFindAnything);
                        }
                    }
                    
                    Console.WriteLine(MessageTotalBits);
                    if (totalBits > 200)
                    {
                        Console.WriteLine(MessageBigger200);    
                    }
                    else
                    {
                        Console.WriteLine(MessageSmaller200);
                    }
                   break;
            }
